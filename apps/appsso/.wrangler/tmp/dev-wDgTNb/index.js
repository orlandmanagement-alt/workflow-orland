...
// .wrangler/tmp/bundle-IEDtky/checked-fetch.js
...
// .wrangler/tmp/bundle-IEDtky/strip-cf-connecting-ip-header.js
...
// src/routes/auth.ts
var auth = new Hono2();
var SESSION_EXPIRY = 259200;
var COOKIE_OPTS = { domain: ".orlandmanagement.com", path: "/", httpOnly: true, secure: true, sameSite: "None" };
var getPortalUrl = /* @__PURE__ */ __name(async (env, user, sid) => {
  ...
  const now = getNow();
  const payload = { sub: user.id, role: safeRole, sid, exp: now + SESSION_EXPIRY, iat: now };
  const token = await sign2(payload, env.JWT_SECRET || "orland-rahasia-utama-123");
  const params = new URLSearchParams({ token, role: safeRole, user_id: user.id, name: user.full_name, email: user.email });
  return `${baseUrl}/auth/callback?${params.toString()}`;
}, "getPortalUrl");
var getNow = /* @__PURE__ */ __name(() => Math.floor(Date.now() / 1e3), "getNow");
auth.get("/me", async (c) => {
  const sid = getCookie(c, "sid");
  if (!sid)
    return c.json({ status: "error", message: "Tidak ada sesi" }, 401);
  const session = await c.env.DB_SSO.prepare("SELECT * FROM sessions WHERE id=? AND expires_at > ?").bind(sid, getNow()).first();
  if (!session)
    return c.json({ status: "error", message: "Sesi kadaluarsa" }, 401);
  const user = await c.env.DB_SSO.prepare("SELECT id, full_name, email, role, status FROM users WHERE id=?").bind(session.user_id).first();
...
auth.post("/logout", async (c) => {
  const sid = getCookie(c, "sid");
  if (sid)
    await c.env.DB_SSO.prepare("DELETE FROM sessions WHERE id=?").bind(sid).run();
...
auth.post("/register", async (c) => {
  try {
    ...
    if (!isHuman)
      return c.json({ status: "error", message: "Verifikasi keamanan gagal." }, 400);
    const cleanEmail = (body.email || "").trim().toLowerCase();
    const safeRole = (body.role || "talent").toLowerCase();
    const existingUser = await c.env.DB_SSO.prepare("SELECT * FROM users WHERE (email=? OR phone=?) AND status != 'deleted'").bind(cleanEmail, cleanEmail).first();
    if (existingUser)
      return c.json({ status: "error", message: "Email atau No HP sudah terdaftar." }, 400);
    const hashedPw = await hashData(body.password);
    const userId = crypto.randomUUID();
    const now = getNow();
    await c.env.DB_SSO.prepare("INSERT INTO users (id, full_name, email, phone, role, password_hash, status, created_at) VALUES (?,?,?,?,?,?,'pending',?)").bind(userId, body.fullName, cleanEmail, body.phone, safeRole, hashedPw, now).run();
...
auth.post("/verify-activation", async (c) => {
  const body = await c.req.json();
  const now = getNow();
  const tokenRow = await c.env.DB_SSO.prepare("SELECT * FROM otp_requests WHERE code=? AND purpose='activation' AND expires_at > ?").bind(body.token, now).first();
  if (!tokenRow)
    return c.json({ status: "error", message: "Tautan Aktivasi tidak valid/kadaluarsa." }, 400);
  ...
  const user = await c.env.DB_SSO.prepare("SELECT * FROM users WHERE email=?").bind(tokenRow.identifier).first();
...
auth.post("/login-password", async (c) => {
  ...
  const user = await c.env.DB_SSO.prepare("SELECT * FROM users WHERE (email=? OR phone=?) AND status != 'deleted'").bind(id, id).first();
  const now = getNow();
  if (!user)
    return c.json({ status: "error", message: "Kredensial salah." }, 401);
  if (user.locked_until && user.locked_until > now)
    return c.json({ status: "error", message: "Akun terkunci. Coba lagi nanti." }, 429);
...
async function handleOtpVerify(c, actionType) {
  const body = await c.req.json();
  const id = (body.identifier || "").trim().toLowerCase();
  const user = await c.env.DB_SSO.prepare("SELECT * FROM users WHERE (email=? OR phone=?) AND status != 'deleted'").bind(id, id).first();
  if (!user)
  ...
  const now = getNow();
  const otpRow = await c.env.DB_SSO.prepare("SELECT * FROM otp_requests WHERE identifier=? AND code=? AND purpose=? AND expires_at > ?").bind(user.email, body.otp, purp, now).first();
  if (!otpRow)
    return c.json({ status: "error", message: "OTP salah/expired." }, 400);
...
__name(handleOtpVerify, "handleOtpVerify");
...
auth.post("/login-pin", async (c) => {
  ...
  if (user.pin_hash !== hashInput)
    return c.json({ status: "error", message: "PIN salah." }, 401);
  const now = getNow();
  const sid = crypto.randomUUID();
  await c.env.DB_SSO.prepare("INSERT INTO sessions (id, user_id, role, created_at, expires_at) VALUES (?,?,?,?,?)").bind(sid, user.id, user.role, now, now + SESSION_EXPIRY).run();
  setCookie(c, "sid", sid, { ...COOKIE_OPTS, maxAge: SESSION_EXPIRY });
...
auth.post("/reset-password", async (c) => {
  const body = await c.req.json();
  const now = getNow();
  const tokenRow = await c.env.DB_SSO.prepare("SELECT * FROM otp_requests WHERE code=? AND purpose='reset' AND expires_at > ?").bind(body.token, now).first();
  if (!tokenRow)
    return c.json({ status: "error", message: "Token tidak valid/kadaluarsa." }, 400);
  ...
  const user = await c.env.DB_SSO.prepare("SELECT * FROM users WHERE email=?").bind(tokenRow.identifier).first();
  if (user)
    await c.env.DB_SSO.prepare("DELETE FROM sessions WHERE user_id=?").bind(user.id).run();
...
// .wrangler/tmp/bundle-IEDtky/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-IEDtky/middleware-loader.entry.ts
