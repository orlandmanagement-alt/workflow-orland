...
import { Search, CreditCard, ChevronRight, CheckCircle2, AlertCircle, Clock, FileText, Upload, Plus, Loader2 } from 'lucide-react';
import { phase4API } from '@/lib/phase4Api';

// Fallback mock data jika API belum siap
...
export default function FinanceHub() {
  ...
  // Fetch invoices dari API pada component mount
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        ...
        // Call phase4API untuk fetch invoices
        ...
        if (response.status === 'success' && response.data) {
          ...
          console.warn('Invoices API response invalid, using fallback data');
          setInvoices(FALLBACK_INVOICES);
        }
      } catch (err: any) {
        console.error('Failed to fetch invoices:', err);
        setError(err.message || 'Gagal memuat daftar tagihan');
        // Graceful degradation: gunakan mock data
        setInvoices(FALLBACK_INVOICES);
      } finally {
  ...
  const handleUploadProof = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!selectedInvoice || !event.target.files?.[0]) return;
    ...
    try {
      ...
      // Upload bukti transfer ke API
      // Assuming phase4API has uploadInvoiceProof method
      const response = await phase4API.uploadInvoiceProof(selectedInvoice.id, file);
      
      if (response.status === 'success') {
        // Update local state untuk reflect perubahan
        ...
        setSelectedInvoice({ ...selectedInvoice, status: 'processing' as const });
        
        // Reset file input
        event.target.value = '';
      } else {
        setError(response.error || 'Gagal mengunggah bukti transfer');
      }
    } catch (err: any) {
  ...
  return (
    <div className="p-4 sm:p-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white flex items-center gap-3">
            <CreditCard className="text-amber-500" /> Pusat Keuangan
          </h1>
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="pl-10 pr-4 py-2.5 rounded-xl bg-white dark:bg-dark-card border border-slate-200 dark:border-slate-800 focus:ring-2 focus:ring-amber-500 outline-none min-w-[250px] shadow-sm text-sm"
          />
        </div>
      </div>

      {error && (
        <div className="p-4 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 rounded-2xl flex items-gap gap-3">
          <AlertCircle className="text-rose-500 flex-shrink-0" size={20} />
          <div>
            <h4 className="font-bold text-rose-900 dark:text-rose-400">Error Loading Invoices</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Kolom Kiri: Tabel Invoices (1/3 width on desktop) */}
        <div className="md:col-span-1 border border-slate-200 dark:border-slate-800 rounded-3xl bg-white dark:bg-dark-card overflow-hidden shadow-sm flex flex-col h-[calc(100vh-240px)]">
          <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-gradient-to-r from-amber-50 to-transparent dark:from-amber-500/5 dark:to-transparent">
             <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
               <FileText size={18} className="text-amber-500" /> Daftar Tagihan
             </h3>
             <span className="text-xs font-bold bg-white dark:bg-black px-2 py-1 rounded-md shadow-sm border border-slate-200 dark:border-slate-800">
                {isLoadingInvoices ? '...' : `${filteredInvoices.length} Bills`}
             </span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {isLoadingInvoices ? (
              <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                ...
                <p className="text-xs font-bold">Memuat tagihan...</p>
              </div>
            ) : filteredInvoices.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                <FileText className="mb-2 opacity-30" size={32} />
                <p className="text-xs font-bold">Tidak ada tagihan</p>
              </div>
            ) : (
                ...
                <button 
                  ...
                  className={`w-full text-left p-4 rounded-2xl border transition-all flex flex-col gap-2
                    ${selectedInvoice?.id === inv.id 
                      ...
                      : 'border-slate-200 dark:border-slate-800 hover:border-amber-300 bg-white dark:bg-[#121b2b]'}`}
                >
                  ...
                  <div>
                    <h4 className={`font-bold truncate ${selectedInvoice?.id === inv.id ? 'text-amber-900 dark:text-amber-400' : 'text-slate-900 dark:text-white'}`}>
                      {inv.project_name}
                    </h4>
                  ...
                  <div className="mt-2 pt-3 border-t border-slate-100 dark:border-slate-800/50 flex justify-between items-center w-full">
                     ...
                     <ChevronRight size={16} className={selectedInvoice?.id === inv.id ? 'text-amber-500' : 'text-slate-300'} />
                  </div>
                </button>
              ))
            )}
        ...
        <div className="md:col-span-2 rounded-3xl overflow-hidden shadow-sm flex flex-col h-[calc(100vh-240px)] border border-slate-200 dark:border-slate-800 relative">
           
           {selectedInvoice ? (
             <div className="flex-1 overflow-y-auto bg-white dark:bg-dark-card">
               <div className="p-6">
                 ...
                 <div className="mt-8 p-6 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900/50 dark:to-slate-800/30 border border-slate-200 dark:border-slate-800 rounded-2xl">
                    {selectedInvoice.status === 'unpaid' && (
                      <div className="text-center space-y-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Harap transfer sesuai Grand Total ke rekening BCA Orland dan unggah buktinya.</p>
                        </div>
                        <label className="px-6 py-3 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-extrabold rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 mx-auto disabled:opacity-50 cursor-pointer inline-flex">
                           {isUploading ? (
                             <>
                    ...
                    {selectedInvoice.status === 'processing' && (
                      <div className="text-center space-y-4 text-amber-600 dark:text-amber-500">
                        <Clock className="mx-auto animate-pulse" size={32} />
                        <div>
                    ...
                    {selectedInvoice.status === 'paid' && (
                      <div className="text-center space-y-4 text-green-600 dark:text-green-500">
                        ...
                        <div>
                          ...
                          <p className="text-sm opacity-80 mt-1">Terima kasih. Dana pembayaran Talent Fee telah kami terima dan proses Payout talent sedang berjalan.</p>
                        </div>
                      </div>
                    )}
                 </div>
...
const StatusBadge = ({ status }: { status: string }) => {
  switch(status) {
    case 'paid': return <span className="bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md flex items-center gap-1"><CheckCircle2 size={10}/> Paid</span>;
