...
import { ImagePlus, GripVertical, Star, Loader2, Trash2 } from 'lucide-react';
...
export default function MediaPortfolio() {
  ...
  // 2. Mutation untuk Upload File (POST)
  const uploadMutation = useMutation({
    mutationFn: mediaService.uploadMedia,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-media'] }); // Refresh galeri otomatis
    },
    onError: (error: any) => alert(error.message || 'Gagal mengunggah file.'),
  });

  // 3. Mutation untuk Set Main Photo (PATCH)
  ...
  if (isError) throw new Error('Gagal memuat galeri portofolio');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button 
          ...
          className="flex items-center px-4 py-2.5 bg-brand-600 text-white font-bold rounded-xl shadow-lg hover:bg-brand-700 disabled:opacity-70 transition-all w-full sm:w-auto justify-center"
        >
      ...
      {items.length === 0 ? (
        <div className="bg-white dark:bg-dark-card p-10 rounded-3xl border border-slate-200 dark:border-slate-800 border-dashed text-center">
            <h3 className="text-lg font-bold dark:text-white">Galeri Masih Kosong</h3>
            <p className="text-slate-500 text-sm mt-2">Mulai unggah Headshot dan Foto Full Body Anda.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {items.map((media: any) => (
              <div key={media.id} className={`group relative bg-slate-100 dark:bg-slate-800 rounded-3xl h-64 flex flex-col justify-end p-4 transition-all overflow-hidden border-2 ${media.is_main ? 'border-brand-500 shadow-lg shadow-brand-500/20' : 'border-transparent'}`}>
                  ...
                  {/* Tampilkan Gambar (Jika ada URL dari backend) */}
                  {media.url && (
                  ...
                  {/* Tombol Aksi Kiri Atas (Set Main Photo) */}
                  <button 
                  ...
                  <div className="relative z-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md p-3 rounded-xl flex justify-between items-center mt-auto shadow-sm">
                      <div className="flex flex-col">
                        <span className="font-bold text-sm dark:text-white truncate max-w-[120px]">{media.filename || 'Portofolio'}</span>
