...
// SOLUSI ENDPOINT: Rute /me harus di atas rute /:talent_id agar tidak dianggap ID
...
router.put('/me', requireRole(['talent', 'superadmin']), async (c) => {
  ...
  try {
    if (existing) {
      await c.env.DB_CORE.prepare(`UPDATE talents SET full_name=?, category=?, height=?, weight=?, birth_date=?, gender=? WHERE user_id=?`).bind(body.full_name, body.category, body.height, body.weight, body.birth_date, body.gender, userId).run()
    } else {
      const newTalentId = crypto.randomUUID()
      await c.env.DB_CORE.prepare(`INSERT INTO talents (talent_id, user_id, full_name, category, height, weight, birth_date, gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).bind(newTalentId, userId, body.full_name, body.category, body.height, body.weight, body.birth_date, body.gender).run()
    }
...
// --- ENDPOINT ADMIN LAMA (Tidak ada yang dihapus) ---
router.get('/', requireRole(['admin', 'superadmin']), zValidator('query', talentQuerySchema), async (c) => {
  ...
  const l = Math.min(parseInt(limit || '10'), 100); const o = parseInt(offset || '0')
  let query = 'SELECT * FROM talents WHERE is_active = 1'; let params: any[] = []
...
router.post('/', requireRole(['admin', 'superadmin']), zValidator('json', createTalentSchema), async (c) => {
  const body = c.req.valid('json'); const talentId = crypto.randomUUID(); const userId = c.get('userId') 
  try {
  ...
  } catch (e: any) { return c.json({ status: 'error', message: 'Database error' }, 500) }
})

router.put('/:talent_id', requireRole(['admin', 'superadmin']), zValidator('json', updateTalentSchema), async (c) => {
  if (c.req.param('talent_id') === 'me') return c.json({ status: 'error', message: 'Invalid ID' }, 400)
  const id = c.req.param('talent_id'); const body = c.req.valid('json')
